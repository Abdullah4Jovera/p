...
router.put('/update-user/:id', async (req, res) => {
  try {
    ...
    if (pipeline) {
      // Ensure pipelines are handled correctly
      let validPipelines = [];
      
      if (Array.isArray(pipeline)) {
        validPipelines = await Promise.all(
        ...
        if (mongoose.Types.ObjectId.isValid(pipeline)) {
          ...
          const foundPipeline = await Pipeline.findOne({ name: pipeline });
          if (foundPipeline) {
            validPipelines = [foundPipeline._id];
          }
        }
      }
